...
async function processProductReposts() {
  try {
    ...
    const [toRepost] = await db.query("SELECT id, product_id, uploaded_by FROM products WHERE COALESCE(updated_at, uploaded_at) <= DATE_SUB(NOW(), INTERVAL 3 DAY) AND reposted_until IS NULL");
