/*
  ═══════════════════════════════════════════════════════════════════════════
  ECO-SMART POULTRY MONITORING SYSTEM - ESP32 FIRMWARE
  ═══════════════════════════════════════════════════════════════════════════
  
  🔒 DEPLOYMENT MODE: LOCAL-ONLY (NO CLOUD)
  
  ✅ Time Source: RTC DS3231 (I2C pins 21, 22) - NO NTP
  ✅ Backend: Local at 192.168.6.199:5000
  ✅ Database: Local MySQL at localhost:3306
  ✅ Frontend: Local at localhost:5432
  ✅ Internet: NOT REQUIRED (fully offline operation supported)
  
  Data Flow: ESP32 → Local Backend → Local MySQL → Local Frontend
...
// ====================== DATABASE & API CONFIG ======================
// 🔒 CONFIGURATION: LOCAL-ONLY BACKEND
// ✅ All data stays LOCAL - NO CLOUD / NO INTERNET REQUIRED
// ✅ Backend runs on your PC at 192.168.6.199:5000
// ✅ Database (MySQL) on your PC at localhost:3306
...
// When true: Render cloud backend is completely DISABLED
// When true: No internet connection required for operation
...
String createSensorPayload() {
  ...
  if (rtcFound) {
    ...
    char rtcTimeBuffer[30];
    snprintf(rtcTimeBuffer, sizeof(rtcTimeBuffer), "%04d-%02d-%02dT%02d:%02d:%02d",
             now.year(), now.month(), now.day(),
             now.hour(), now.minute(), now.second());
    doc["timestamp"] = rtcTimeBuffer;
  } else {
...
void setup() {
  ...
  if (rtcFound) {
    ...
    Serial.println("[RTC] DS3231 not detected; time not set");
  }
...
void loop() {
  ...
  if (Serial.available()) {
    ...
    } else if (cmd.startsWith("SET RTC ")) {
      // Expecting: SET RTC YYYY-MM-DDTHH:MM:SS  (ISO 8601)
      String t = cmd.substring(8); // rest after "SET RTC "
      t.trim();
      if (t.length() >= 19) {
        int yr = t.substring(0,4).toInt();
        int mo = t.substring(5,7).toInt();
        int da = t.substring(8,10).toInt();
        int hh = t.substring(11,13).toInt();
        int mi = t.substring(14,16).toInt();
        int ss = t.substring(17,19).toInt();
        if (yr >= 2000 && mo >=1 && mo <=12 && da >=1 && da <=31) {
          rtc.adjust(DateTime(yr, mo, da, hh, mi, ss));
          rtcFound = true;
          Serial.print("[RTC] DS3231 updated to: ");
          Serial.println(t);
        } else {
          Serial.println("[RTC] Invalid date provided");
        }
      } else {
        Serial.println("[RTC] Timestamp too short. Use YYYY-MM-DDTHH:MM:SS");
      }
...
void initRTC() {
  ...
  if (!rtc.begin()) {
    Serial.println("[RTC] DS3231 NOT FOUND - check I2C connections on pins 21(SDA), 22(SCL)");
    rtcFound = false;
    return;
  }
  
  rtcFound = true;
  Serial.println("[RTC] ✓ DS3231 detected on I2C");
