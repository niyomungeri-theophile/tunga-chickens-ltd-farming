      ...
      <div className={`fixed left-0 top-0 h-screen bg-[#020c02] text-[#39ff14] z-50 flex flex-col p-6 transition-transform duration-500 border-r border-[#39ff14]/10 w-72 ${isOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0`}>
        ...
        <div className="pt-6 border-t border-slate-100 dark:border-white/5 mt-6">
          ...
          <Link 
            to="/profile"
            onClick={toggle}
            className="w-full flex items-center space-x-3 p-4 rounded-2xl text-[#1a7a1a] hover:bg-[#39ff14]/10 hover:text-[#39ff14] transition-all font-black text-xs uppercase tracking-widest mb-3"
          >
            <span>👤</span>
    ...
    <div className={`min-h-screen transition-colors duration-500 ${theme === 'dark' ? 'bg-slate-950 text-slate-100' : 'bg-slate-50 text-slate-900'}`}>
      ...
      {user && isPublicRole && (
        <div className="fixed top-6 right-6 z-[60]">
          <button
            ...
            className="flex items-center gap-2 px-4 py-3 rounded-2xl bg-white dark:bg-slate-900 shadow-xl border border-slate-100 dark:border-white/10 text-slate-700 dark:text-slate-100 font-black text-xs"
          >
            <LogOut size={18} />
            <span>{t('sign_out')}</span>
          </button>
      ...
      <div className={`${user && !isPublicRole ? 'md:ml-72 pt-20 md:pt-0' : 'pt-20'} transition-all duration-500 min-h-screen relative`}>
        <Routes>
          ...
          <Route path="/login" element={!user ? <Login /> : <Navigate to={role === 'admin' ? '/users' : role === 'supervisor' ? '/supervisor-dashboard' : isMarketplace ? '/customer-dashboard' : '/farmer-dashboard'} />} />
          ...
          <Route path="/request-form" element={user ? (isPublicRole ? <Navigate to="/" replace /> : role === 'admin' ? <RequestForm /> : <Navigate to={isSupervisor ? '/supervisor-dashboard' : isMarketplace ? '/customer-dashboard' : '/farmer-dashboard'} />) : <Navigate to="/login" />} />
          <Route path="/image-form" element={user ? (isPublicRole ? <Navigate to="/" replace /> : role === 'admin' ? <ImageForm /> : <Navigate to={isSupervisor ? '/supervisor-dashboard' : isMarketplace ? '/customer-dashboard' : '/farmer-dashboard'} />) : <Navigate to="/login" />} />
          <Route path="/seller-requests" element={user ? (isPublicRole ? <Navigate to="/" replace /> : role === 'admin' ? <SellerRequests /> : <Navigate to={isSupervisor ? '/supervisor-dashboard' : isMarketplace ? '/customer-dashboard' : '/farmer-dashboard'} />) : <Navigate to="/login" />} />
          ...
          <Route path="/users" element={user ? (isPublicRole ? <Navigate to="/" replace /> : isAdminLikeFinal ? <UserManagement /> : <Navigate to={isMarketplace ? '/customer-dashboard' : '/farmer-dashboard'} />) : <Navigate to="/login" />} />
          <Route path="/devices" element={user ? (isPublicRole ? <Navigate to="/" replace /> : isAdminLikeFinal ? <AdminDeviceManagement /> : <Navigate to={isMarketplace ? '/customer-dashboard' : '/farmer-dashboard'} />) : <Navigate to="/login" />} />
